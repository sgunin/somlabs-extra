...
do
    case $fsl_setup_flag in
        ...
        e)
            ...
            if [ "$BACKEND" = "vs" ]; then
                if [ -z "$DISTRO" ]; then
                    FSLDISTRO='visionsom6ull'
                    echo -e "\n Using visionsom6ull distro"
                elif [ ! "$DISTRO" = "visionsom6ull" ]; then
                    echo -e "\n DISTRO specified as 'visionsom6ull' conflicts with -e fb. Please use just one or the other."
                    fsl_setup_error='true'
                fi
            else
                echo -e "\n Invalid backend specified with -e. Use 'vs'"
                fsl_setup_error='true'
            fi
           ;;
